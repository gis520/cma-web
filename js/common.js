...
common.addWarnMarker = function (data, arrMarker) {
  ...
  var m = maphelper.addClickableMarker(
    ...
    function () {
      // 鼠标悬停事件
      var title =
        data.name + '发布' + data.codename + data.signallevel + '预警信号';
      var values =
...
common.addDisasterMarker = function (data, arrMarker) {
  ...
  var h = 34;
  var url =
    ctxStatic + '/ultra/img/gis/disasterWarning/' + data.MAINTYPE + '.png?v=1';
  var m = maphelper.addClickableMarker(
    common.translateNum(data.lon),
    common.translateNum(data.lat),
    {
