...
staPoint.drawArea = function (km, lat, lon) {
  ...
  staPoint.mapDiv = maphelper.addDiv(
    lon + 0.8,
    lat,
...
staPoint.prcsData = function (result, flag, funitemmenuid, lon, lat) {
  ...
  if (totalPageNum != 0) {
    for (let i = 0; i < totalPageNum; i++) {
      let tempArrPageRecord = syntheical.groupRecord(
        publicPageSize * i,
        publicPageSize * (i + 1),
        list,
      );
...
staPoint.addLocationMark = function (lat, lon) {
  ...
  staPoint.removeLocationMark = maphelper.addClickableMarker2(
  ...
  let chart_ = Highcharts.chart('me_lineGraph', {
    ...
    xAxis: {
      ...
      labels: {
        ...
        formatter: function () {
          ...
          if (value.length > 10) {
            value =
              value.substring(6, 8) + '日' + value.substring(8, 10) + '时';
          }
          return value;
        },
      },
    ...
    yAxis: {
      ...
      title: {
        align: 'middle',
        text:
          resultall['name' + index] + '(' + resultall['units' + index] + ')',
        //rotation: 0,
        x: 6,
        y: 0,
      },
        ...
        if (globalParam.staid == '115010201') {
          ...
          for (let i = 0; i < emgcy.arrWarning.length; i++) {
            if (strTime == emgcy.arrWarning[i].time) {
              html += '  ';
              if (
                emgcy.arrWarning[i].codename != undefined &&
                emgcy.arrWarning[i].codename != null &&
                emgcy.arrWarning[i].codename != ''
              ) {
                html += emgcy.arrWarning[i].codename;
              }
              if (
                emgcy.arrWarning[i].signallevel != undefined &&
                emgcy.arrWarning[i].signallevel != null &&
                emgcy.arrWarning[i].signallevel != ''
              ) {
